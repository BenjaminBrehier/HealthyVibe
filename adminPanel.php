    ...
    <section>
        ...
        ?>
            ...
            <table>
                ...
                <?php
                $co = mysqli_connect(DB_HOST, DB_USERNAME, DB_PASSWORD, DB_NAME);
                $result = $co->query("SELECT M.idMessage, M.date, M.contenu, (SELECT email FROM utilisateur U WHERE U.idUtilisateur = M.idUtilisateur) AS email FROM messagedirect M ORDER BY M.date DESC");
                while ($row = $result->fetch_object()) {
                ?>
                    <tr>
